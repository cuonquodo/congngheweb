...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const [rooms] = await db.query('SELECT * FROM phong_tro WHERE id = ?', [room_id]);
        if (rooms.length === 0) {
        ...
        await db.query(query, [userId, room_id, ho_ten, ngay_sinh || null, so_nguoi_o || 1]);

        // Gửi thông báo cho TẤT CẢ chủ trọ
        const [landlords] = await db.query('SELECT id FROM nguoi_dung WHERE role = ?', ['chu_tro']);
        const notificationMessage = `Có yêu cầu đặt phòng mới từ ${ho_ten}`;
        
        for (const landlord of landlords) {
            await db.query(
                'INSERT INTO notifications (user_id, type, message, is_read, created_at) VALUES (?, ?, ?, FALSE, NOW())',
                [landlord.id, 'booking_pending', notificationMessage]
            );
        }
...
router.post('/:bookingId/cancel', authMiddleware, async (req, res) => {
    try {
        ...
        // Tạo thông báo cho TẤT CẢ chủ trọ
        const [landlords] = await db.query('SELECT id FROM nguoi_dung WHERE role = ?', ['chu_tro']);
        const notificationMessage = `Khách hàng ${booking.ho_ten} đã hủy đặt phòng "${booking.room_title}"`;
        
        for (const landlord of landlords) {
            await db.query(
                'INSERT INTO notifications (user_id, type, message, is_read, created_at) VALUES (?, ?, ?, FALSE, NOW())',
                [landlord.id, 'booking_cancel', notificationMessage]
            );
        }
...
router.post('/:bookingId/confirm', authMiddleware, async (req, res) => {
    try {
        ...
        const updateQuery = `
            UPDATE bookings 
            SET trang_thai = 'confirmed', updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        `;
        await db.query(updateQuery, [bookingId]);
        ...
        await db.query(
            'INSERT INTO notifications (user_id, type, message, is_read, created_at) VALUES (?, ?, ?, FALSE, NOW())',
            [booking.user_id, 'booking_confirmed', `Đặt phòng "${booking.room_title}" đã được xác nhận`]
        );
...
router.post('/:bookingId/reject', authMiddleware, async (req, res) => {
    try {
        ...
        await db.query(
            'INSERT INTO notifications (user_id, type, message, is_read, created_at) VALUES (?, ?, ?, FALSE, NOW())',
            [booking.user_id, 'booking_rejected', `Đặt phòng "${booking.room_title}" đã bị từ chối. Lý do: ${reason || 'Không có lý do'}`]
        );
...
router.post('/:bookingId/confirm-cancel', authMiddleware, async (req, res) => {
    try {
        ...
        const updateQuery = `
            UPDATE bookings 
            SET trang_thai = 'cancelled_confirmed', updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        `;
        await db.query(updateQuery, [bookingId]);
        ...
        await db.query(
            'INSERT INTO notifications (user_id, type, message, is_read, created_at) VALUES (?, ?, ?, FALSE, NOW())',
            [booking.user_id, 'cancel_confirmed', `Yêu cầu hủy phòng "${booking.room_title}" đã được xác nhận`]
        );
