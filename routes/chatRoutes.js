...
router.post('/send', authMiddleware, async (req, res) => {
    try {
        ...
        if (io) {
            ...
            // Nếu người gửi là sinh viên, gửi thông báo cho TẤT CẢ chủ trọ
            if (req.user.role === 'sinh_vien') {
                ...
                for (const landlord of landlords) {
                    await Notification.create(landlord.id, 'message', notificationMessage);
                    
                    io.to(`user_${landlord.id}`).emit('new_notification', {
                        message: notificationMessage,
                        type: 'message'
                    });
                }
            } else {
                // Nếu chủ trọ gửi tin nhắn, chỉ thông báo cho người nhận
                const notificationMessage = `${req.user.ho_ten} đã gửi tin nhắn mới`;
                await Notification.create(receiverId, 'message', notificationMessage);
