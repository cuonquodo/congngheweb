...
exports.createPhongTro = async (req, res) => {
    ...
    // 2. Lấy dữ liệu từ body - QUAN TRỌNG: Phải có noi_that_ids ở đây
    const {
        tieu_de, dia_chi, phuong_xa, thanh_pho, 
        gia_tien, dien_tich, mo_ta, vi_do, kinh_do,
        noi_that_ids // <--- Thêm dòng này để định nghĩa biến
    } = req.body;

    const sqlPhong = `
        INSERT INTO phong_tro 
        (chu_phong_id, tieu_de, dia_chi, phuong_xa, thanh_pho, gia_tien, dien_tich, mo_ta, vi_do, kinh_do)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    try {
        ...
        if (noi_that_ids) {
            let finalIds = Array.isArray(noi_that_ids) ? noi_that_ids : [noi_that_ids];
